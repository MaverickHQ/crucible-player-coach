...
def half_kelly(stats: TradeStats, cap: float | None = None) -> float:
    ...
    return ``0.0``.
    """
    if stats.count == 0 or stats.avg_win <= 0.0:
        return 0.0

    if stats.avg_loss <= 0.0:
        # No losses yet → payoff ratio is unbounded, so (1-W)/b → 0 and full
        # Kelly → the win rate. Sizing to zero here is wrong precisely when the
        # record is strongest; the cap still bounds it downstream.
        full = stats.win_rate
    else:
