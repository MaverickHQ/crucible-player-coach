...
class ConstraintSchema:
    ...
    # Feature 13: soft warning threshold below the hard consistency limit.
    consistency_warn_pct: float = 0.40
    ...
    def consistency_status(self, day_pnl: float, total_pnl: float) -> str:
        """Graduated consistency signal by today's share of total profit (F13):
        ``ok`` < warn ≤ ``approaching`` ≤ limit < ``breached``. A non-positive
    ...
    def from_dict(cls, data: dict[str, Any]) -> ConstraintSchema:
        return cls(
            ...
            consistency_warn_pct=data.get("consistency_warn_pct", 0.40),
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "consistency_warn_pct": self.consistency_warn_pct,
